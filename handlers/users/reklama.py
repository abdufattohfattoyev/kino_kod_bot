...
class Advertisement:
    ...
    async def start(self):
        ...
        for i, user in enumerate(users):
            ...
            await self._send_with_retry(user[1])
...
def _is_admin(telegram_id: int) -> bool:
    return telegram_id in ADMINS or user_db.check_if_admin(telegram_id)
...
async def handle_time_input(message: types.Message, state: FSMContext):
    try:
        ...
        await message.reply("❌ Noto'g'ri format. HH:MM ko'rinishida kiriting. Masalan: <b>14:30</b>",
                            parse_mode="HTML")
